...
# =========================================================
# 十一、根据目标像素方向，从激光中取对应方向距离
# =========================================================
def get_laser_depth_for_target(scan_msg, target_u, camera_matrix, angle_window=3):
    """
    根据目标在图像中的水平像素位置，估算其相对相机光轴的水平偏角，
    再从激光雷达中取对应方向附近的距离作为“近似深度”
    """
...
def rgb_callback(msg):
    ...
    global depth_image, camera_matrix, latest_scan
    ...
    if current_color_name == "red":
        mask1 = cv2.inRange(hsv_image, (0, satu_min, val_min), (10, satu_max, val_max))
        mask2 = cv2.inRange(hsv_image, (170, satu_min, val_min), (179, satu_max, val_max))
        ...
        if area >= 100:
            ...
            if depth_image is not None and camera_matrix is not None:
                ...
                if 0 <= target_u < depth_w and 0 <= target_v < depth_h:
                    Z_depth = get_average_depth(depth_image, target_u, target_v, window_size=5)
                    if Z_depth is not None:
                        measured_Z = Z_depth
                        distance_source = "depth"

            # -------------------------------------------------
            # 2) 深度无效时，用激光在目标方向上的距离替代
            # -------------------------------------------------
            if measured_Z is None and latest_scan is not None and camera_matrix is not None:
                Z_laser = get_laser_depth_for_target(latest_scan, target_u, camera_matrix, angle_window=3)
                if Z_laser is not None:
                    measured_Z = Z_laser
                    distance_source = "laser"

            # -------------------------------------------------
            # 3) 如果得到可用距离（无论深度还是激光），就继续坐标计算
            # -------------------------------------------------
            if measured_Z is not None and camera_matrix is not None:
                ...
                rospy.loginfo_throttle(
                    1.0,
                    "相机坐标系(%s): X=%.3f m, Y=%.3f m, Z=%.3f m",
                    distance_source, X, Y, Z
                )

                text_cam = "Cam({}): X:{:.2f} Y:{:.2f} Z:{:.2f}m".format(distance_source, X, Y, Z)
                ...
                try:
                    if tf_buffer is not None and tf_buffer.can_transform(
                        ...
                        src_text = "DistSrc: {}".format(distance_source)
                        ...
                        cv2.putText(cv_image, hori_text, (10, 90),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
                        cv2.putText(cv_image, src_text, (10, 150),
                ...
                # 4) 深度和激光都无效：退回纯视觉兜底
                # -------------------------------------------------
                rospy.logwarn_throttle(1.0, "深度和激光均无效，使用纯视觉引导接近目标")
...
if __name__ == "__main__":
    ...
    rospy.loginfo("===== TurtleBot3 HSV 状态机跟踪节点（激光替代深度版）启动 =====")
